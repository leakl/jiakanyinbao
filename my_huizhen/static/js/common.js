...
var Is={}
    ...
    Is.ios = /iPhone/.test(navigator.userAgent);
(function(){
    var S = {};
    S.init =(function(){
        //链接
        ...
        //清空内容
        $('.ui-form-item').on('input','input',function(){
            var o = $(this);
            var close = $(this).parent().find('.ui-icon-close')
                close.show();
        ...
        if(Is.ios){
            var url = window.location.origin +"/static/js/lib/fastclick.min.js"
    ...
    S.tipsTimer = null;
    S.tips = function(str,time){
        clearTimeout(S.tipsTimer)
        var time = time || 1200;
        var html = '<div class="ui-poptips animated fadeInUp"><div class="ui-poptips-cnt">'+str+'</div> </div>';
        if(!$('.ui-poptips').size()){
            $('body').append(html).addClass('pointer-events');
        }else{
            $('.ui-poptips-cnt').html(str);
        }
        S.tipsTimer = setTimeout(function(){
            $('.ui-poptips').remove();
            $('body').removeClass('pointer-events');
        },time)
    }
    S.debug = (function(){
           // $('body').append('<button class="ui-btn" onclick="window.location.reload()">刷新</button>')
        }())
    S.clear =function(){
        $('[src]').each(function(){
    ...
    S.confirm = function(string){
//        var html = '<div class="ui-dialog">' +
//            '<div class="ui-dialog-cnt">' +
//            '<header class="ui-dialog-hd ui-border-b">' +
//            '<h3>新手任务</h3><i class="ui-dialog-close" data-role="button"></i> </header>' +
//            '<div class="ui-dialog-bd"><h4>标题标题</h4> <div>开通年费QQ会员即可领取欢乐斗地主感恩节回馈礼包！</div> </div>' +
